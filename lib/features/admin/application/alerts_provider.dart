...
final adminAlertsProvider = Provider<List<AdminAlert>>((ref) {
  ...
  final overdue = visits
      .where((v) => v.visit.status < 2 && v.visit.scheduledAt.isBefore(today))
      .toList();
  for (var v in overdue) {
    alerts.add(
      AdminAlert(
        ...
        message:
            '${v.visit.companyName} prevista per il ${DateFormat('dd/MM').format(v.visit.scheduledAt)}',
  ...
  for (var v in upcoming) {
    alerts.add(
      AdminAlert(
        ...
        message:
            'Visita ${v.visit.companyName} tra ${v.visit.scheduledAt.difference(today).inDays} giorni',
  ...
  final Map<String, Map<String, int>> inspectorWorkload =
      {}; // {Date: {Inspector: count}}
  for (var v in visits) {
    if (v.visit.status < 2 && v.visit.inspectorName.isNotEmpty) {
      ...
      inspectorWorkload[dateKey]![v.visit.inspectorName] =
          (inspectorWorkload[dateKey]![v.visit.inspectorName] ?? 0) + 1;
    }
  }
  ...
  inspectorWorkload.forEach((date, workloads) {
    workloads.forEach((name, count) {
      if (count > 2) {
        // More than 2 visits a day is a warning
        alerts.add(
          AdminAlert(
  ...
  for (var i in inactive) {
    alerts.add(
      AdminAlert(
        ...
        message:
            "L'ispettore ${i.fullName} non ha ancora ricevuto le credenziali",
