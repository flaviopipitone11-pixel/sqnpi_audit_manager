...
final inspectorsWorkloadProvider = StreamProvider.autoDispose<List<InspectorWorkload>>((ref) {
  final db = ref.watch(appDatabaseProvider);
  
  final inspectorsStream = (db.select(db.inspectors)..orderBy([(t) => OrderingTerm.asc(t.fullName)])).watch();
  ...
  return inspectorsStream.asyncMap((inspectors) async {
    ...
    return inspectors.map((inspector) {
      final inspectorVisits = visits.where((v) => v.inspectorName == inspector.fullName).toList();
