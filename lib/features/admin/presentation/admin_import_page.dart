...
class _AdminImportPageState extends ConsumerState<AdminImportPage> {
    ...
    try {
      ...
      for (var table in excel.tables.keys) {
        ...
        for (int i = 1; i < sheet.maxRows; i++) {
          ...
          final visitId = row[0]?.value?.toString() ?? 'V-${DateTime.now().millisecondsSinceEpoch}-$i';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Importazione Massiva', style: TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF1A237E))),
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              ...
              style: TextStyle(fontSize: 16, color: Colors.blueGrey.shade600, height: 1.6, fontWeight: FontWeight.w500),
            ),
                ...
                children: [
                  Row(
                    children: [
                      ...
                      const Text('Struttura File Ottimale', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 20, color: Color(0xFF1A237E))),
                    ],
                  ),
                        ...
                        child: Column(
                          ...
                          children: [
                            if (_isImporting)
                              const Column(
                                children: [
                                  CircularProgressIndicator(strokeWidth: 3, color: Color(0xFF1A237E)),
                                  SizedBox(height: 24),
                                  Text('ELABORAZIONE IN CORSO...', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 13, color: Color(0xFF1A237E), letterSpacing: 1)),
                                ],
                              )
                            else ...[
                              Container(
                              ...
                              const SizedBox(height: 24),
                              const Text('Seleziona il file Excel', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 22, color: Color(0xFF1A237E), letterSpacing: -0.5)),
                              const SizedBox(height: 8),
                              Text('Trascina qui o clicca per sfogliare', style: TextStyle(color: Colors.blueGrey.shade400, fontWeight: FontWeight.w500)),
                              if (_importedCount > 0) ...[
                                const SizedBox(height: 12),
                                Container(
            ...
            if (_statusMessage != null) ...[
              ...
              Center(
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: _statusMessage!.contains('Errore') ? Colors.red.shade50 : Colors.green.shade50,
                  ...
                  child: Row(
                    ...
                    children: [
                      Icon(
                        _statusMessage!.contains('Errore') ? Icons.error_outline : Icons.check_circle_outline,
                        color: _statusMessage!.contains('Errore') ? Colors.red : Colors.green,
                      ),
                      const SizedBox(width: 12),
                      Flexible(child: Text(_statusMessage!, style: TextStyle(color: _statusMessage!.contains('Errore') ? Colors.red.shade900 : Colors.green.shade900, fontWeight: FontWeight.bold))),
                    ],
                  ),
                ),
              ),
      ...
      child: Row(
        ...
        children: [
          Container(
            ...
            child: Text(col, style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 11, color: Colors.white)),
          ),
          const SizedBox(width: 12),
          Text(desc, style: const TextStyle(color: Color(0xFF1A237E), fontWeight: FontWeight.w600, fontSize: 13)),
        ],
      ),
    );
  }
...
class _DashPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    const dashWidth = 8.0;
    const dashSpace = 6.0;
    final rrect = RRect.fromRectAndRadius(Offset.zero & size, const Radius.circular(32));
